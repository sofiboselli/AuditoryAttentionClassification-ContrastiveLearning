...
class SigLIP(torch.nn.Module):
    def __init__(self,scale,temp,bias=-10,acc=False):
        super().__init__()
        self.scale = scale
        self.temp = temp
        self.acc = acc
...
class SubjectLayers(nn.Module):
    def __init__(self, channels, n_subjects):
        super().__init__()
        self.weights = nn.Parameter(torch.randn(n_subjects, channels, channels))
        self.weights.data *= 1 / channels**0.5
...
class EEGConv(nn.Module):
    ...
    def __init__(self, embedding, dropout, kernels, eeg_channels):
        ...
        self.sublayer = SubjectLayers(eeg_channels,33)
        self.mh = nn.ModuleList([nn.MultiheadAttention(embedding,2,batch_first=True) for k in self.kernels])
...
class Encode(pl.LightningModule):
    ...
    def _initialize_modules(self, embedding_size, eeg_drop, env_drop, kernels, rec_embedding, eeg_channels):
        ...
        self.afn = SigLIP(scale=True, temp=self.temp, acc=True, bias=self.bias)
        self.los = PearsonR()
